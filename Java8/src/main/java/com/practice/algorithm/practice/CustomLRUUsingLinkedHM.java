...
public class CustomLRUUsingLinkedHM<K, V> extends LinkedHashMap<K, V> {
	private static int maxEntries = 5;
	private static final int DEFAULT_INITIAL_CAPACITY = 16;
	private static final float DEFAULT_LOAD_FACTOR = 0.75f;

	public CustomLRUUsingLinkedHM(int initialCapacity, float loadFactor, int maxEntries) {
		super(initialCapacity, loadFactor, true);
		this.maxEntries = maxEntries;
	}

	public CustomLRUUsingLinkedHM(int initialCapacity, int maxEntries) {
		this(initialCapacity, DEFAULT_LOAD_FACTOR, maxEntries);
	}

	public CustomLRUUsingLinkedHM(int maxEntries) {
